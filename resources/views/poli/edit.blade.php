...
@section('content')
    <div class="page-header">
        <div>
            <h1 class="page-title">Edit Data Poli</h1>
    ...
    <div class="card" style="max-width: 700px;">
        <form method="POST" action="{{ route('poli.update', $poli->id_poli) }}" class="form">
            @csrf
            @method('PUT')
            ...
            <div class="form-section">
                ...
                <div class="form-group">
                    ...
                    <textarea id="deskripsi" name="deskripsi" rows="4" class="form-control">{{ old('deskripsi', $poli->deskripsi) }}</textarea>
                </div>

                <div class="form-row">
                    <div class="form-group">
                        <label for="lokasi">Lokasi Ruangan</label>
                        <input type="text" id="lokasi" name="lokasi" value="{{ old('lokasi', $poli->lokasi) }}"
                            class="form-control">
                    </div>
                    <div class="form-group">
                        <label for="telepon">Telepon</label>
                        <input type="tel" id="telepon" name="telepon" value="{{ old('telepon', $poli->telepon) }}"
                            class="form-control">
                    </div>
                </div>
            </div>
            ...
            <div class="form-section">
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="time" id="jam_buka" name="jam_buka" value="{{ old('jam_buka', $poli->jam_buka) }}"
                            class="form-control">
                    </div>
                    <div class="form-group">
                        <label for="jam_tutup">Jam Tutup</label>
                        <input type="time" id="jam_tutup" name="jam_tutup"
                            value="{{ old('jam_tutup', $poli->jam_tutup) }}" class="form-control">
                        @error('jam_tutup')
                            <span class="form-error">{{ $message }}</span>
                        @enderror
                    </div>
            ...
            <div class="form-section">
                ...
                <div class="form-group">
                    ...
                    <select id="status" name="status" class="form-control @error('status') is-invalid @enderror">
                        <option value="Aktif" {{ old('status', $poli->status) == 'Aktif' ? 'selected' : '' }}>Aktif
                        </option>
                        <option value="Non-Aktif" {{ old('status', $poli->status) == 'Non-Aktif' ? 'selected' : '' }}>
                            Non-Aktif</option>
                    </select>
                    @error('status')
                        <span class="form-error">{{ $message }}</span>
