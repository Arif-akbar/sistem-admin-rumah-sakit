...
@section('content')
    ...
    <div class="card" style="max-width: 700px;">
        <form method="POST" action="{{ route('dokter.update', $dokter->id_dokter) }}" class="form">
            @csrf
            @method('PUT')
                    ...
                    <div class="form-group">
                        ...
                        <select id="id_spesialis" name="id_spesialis"
                            ...
                            @foreach ($spesialis as $s)
                                <option value="{{ $s->id }}"
                                    {{ old('id_spesialis', $dokter->id_spesialis) == $s->id ? 'selected' : '' }}>
                                    {{ $s->nama_spesialis }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select id="id_poli" name="id_poli" class="form-control @error('id_poli') is-invalid @enderror">
                            @foreach ($polis as $p)
                                <option value="{{ $p->id_poli }}"
                                    {{ old('id_poli', $dokter->id_poli) == $p->id_poli ? 'selected' : '' }}>
                                    {{ $p->nama_poli }}
                                </option>
                            @endforeach
                        </select>
            ...
            <div class="form-section">
                ...
                <div class="form-group">
                    ...
                    <textarea id="alamat" name="alamat" rows="3" class="form-control">{{ old('alamat', $dokter->alamat) }}</textarea>
                </div>

                <div class="form-row">
                    <div class="form-group">
                        <label for="telepon">Nomor Telepon</label>
                        <input type="tel" id="telepon" name="telepon" value="{{ old('telepon', $dokter->telepon) }}"
                            class="form-control">
                    </div>
                    <div class="form-group">
            ...
            <div class="form-section">
                ...
                <div class="form-group">
                    ...
                    <select id="status" name="status" class="form-control @error('status') is-invalid @enderror">
                        <option value="Aktif" {{ old('status', $dokter->status) == 'Aktif' ? 'selected' : '' }}>Aktif
                        </option>
                        <option value="Non-Aktif" {{ old('status', $dokter->status) == 'Non-Aktif' ? 'selected' : '' }}>
                            Non-Aktif</option>
                    </select>
                    @error('status')
                        <span class="form-error">{{ $message }}</span>
